...
template <class CellData>
void plGameGrid<CellData>::ComputeWorldSpaceCorners(plVec3* pCorners) const
{
  pCorners[0] = m_vWorldSpaceOrigin;
  pCorners[1] = m_vWorldSpaceOrigin + m_mRotateToWorldspace * plVec3(m_uiGridSizeX * m_vLocalSpaceCellSize.x, 0, 0);
  pCorners[2] = m_vWorldSpaceOrigin + m_mRotateToWorldspace * plVec3(0, m_uiGridSizeY * m_vLocalSpaceCellSize.y, 0);
  pCorners[3] = m_vWorldSpaceOrigin + m_mRotateToWorldspace * plVec3(m_uiGridSizeX * m_vLocalSpaceCellSize.x, m_uiGridSizeY * m_vLocalSpaceCellSize.y, 0);
}

template <class CellData>
plResult plGameGrid<CellData>::Serialize(plStreamWriter& stream) const
{
  ...
  PL_SUCCEED_OR_RETURN(stream.WriteArray(m_Cells));

  return PL_SUCCESS;
}

template <class CellData>
plResult plGameGrid<CellData>::Deserialize(plStreamReader& stream)
{
  ...
  stream >> m_vLocalSpaceCellSize;
  stream >> m_vInverseLocalSpaceCellSize;
  PL_SUCCEED_OR_RETURN(stream.ReadArray(m_Cells));
