...
plResource* plResourceManager::GetResource(const plRTTI* pRtti, plStringView sResourceID, bool bIsReloadable)
{
  ...
  PL_ASSERT_DEBUG(pRtti != nullptr, "There is no RTTI information available for the given resource type '{0}'", PL_PP_STRINGIFY(ResourceType));
  PL_ASSERT_DEBUG(pRtti->GetAllocator() != nullptr && pRtti->GetAllocator()->CanAllocate(), "There is no RTTI allocator available for the given resource type '{0}'", PL_PP_STRINGIFY(ResourceType));
