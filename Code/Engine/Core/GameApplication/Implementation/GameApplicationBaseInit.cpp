...
void plGameApplicationBase::Init_FileSystem_ConfigureDataDirs()
{
  ...
  plFileSystem::CreateDirectoryStructure(sUserDataPath).AssertSuccess();

  plString writableBinRoot = ">appdir/";
  plString shaderCacheRoot = ">sdk/Output/";
  ...
  plFileSystem::AddDataDirectory("", "GameApplicationBase", ":", plDataDirUsage::ReadOnly).AssertSuccess();

  // ":bin/" : writing to the binary directory
  plFileSystem::AddDataDirectory(writableBinRoot, "GameApplicationBase", "bin", plDataDirUsage::AllowWrites).AssertSuccess();

  // ":shadercache/" for reading and writing shader files
  plFileSystem::AddDataDirectory(writableBinRoot, "GameApplicationBase", "shadercache", plDataDirUsage::AllowWrites).AssertSuccess();

  // ":appdata/" for reading and writing app user data
  plFileSystem::AddDataDirectory(sUserDataPath, "GameApplicationBase", "appdata", plDataDirUsage::AllowWrites).AssertSuccess();

  // ":base/" for reading the core engine files
  plFileSystem::AddDataDirectory(GetBaseDataDirectoryPath(), "GameApplicationBase", "base", plDataDirUsage::ReadOnly).IgnoreResult();

  // ":project/" for reading the project specific files
  plFileSystem::AddDataDirectory(GetProjectDataDirectoryPath(), "GameApplicationBase", "project", plDataDirUsage::ReadOnly).IgnoreResult();
  ...
  // ":plugins/" for plugin specific data (optional, if it exists)
  {
    ...
    {
      plFileSystem::AddDataDirectory(">sdk/Data/Plugins", "GameApplicationBase", "plugins", plDataDirUsage::ReadOnly).IgnoreResult();
    }
  }
