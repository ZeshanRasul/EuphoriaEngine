...
public:
  ...
  plTypedComponentHandle<ComponentType> CreateComponent(plGameObject* pOwnerObject, ComponentType*& out_pComponent);
...
public:                                                                                                                 \
  ...
  virtual plWorldModuleTypeId GetTypeId() const override                                                                \
  {                                                                                                                     \
    return s_TypeId;                                                                                                    \
  }                                                                                                                     \
  static PL_ALWAYS_INLINE plWorldModuleTypeId TypeId()                                                                  \
  {                                                                                                                     \
    return s_TypeId;                                                                                                    \
  }                                                                                                                     \
  plTypedComponentHandle<componentType> GetHandle() const                                                               \
  {                                                                                                                     \
    return plTypedComponentHandle<componentType>(plComponent::GetHandle());                                             \
  }                                                                                                                     \
  virtual plComponentMode::Enum GetMode() const override;                                                               \
  static plTypedComponentHandle<componentType> CreateComponent(plGameObject* pOwnerObject, componentType*& pComponent); \
...
#define PL_BEGIN_COMPONENT_TYPE(componentType, version, mode)                                                                            \
  ...
  plComponentMode::Enum componentType::GetMode() const                                                                                   \
  {                                                                                                                                      \
    return mode;                                                                                                                         \
  }                                                                                                                                      \
  plTypedComponentHandle<componentType> componentType::CreateComponent(plGameObject* pOwnerObject, componentType*& out_pComponent)       \
  {                                                                                                                                      \
...
///
/// \see PL_BEGIN_DYNAMIC_REFLECTED_TYPE
#define PL_BEGIN_ABSTRACT_COMPONENT_TYPE(componentType, version) PL_BEGIN_ABSTRACT_DYNAMIC_REFLECTED_TYPE(componentType, version)
...
#define PL_END_ABSTRACT_COMPONENT_TYPE PL_END_ABSTRACT_DYNAMIC_REFLECTED_TYPE
