...
public:
  ...
  template <typename ComponentType>
  [[nodiscard]] bool TryGetComponent(const plTypedComponentHandle<ComponentType>& hComponent, ComponentType*& out_pComponent)
  {
    return TryGetComponent<ComponentType>(static_cast<const plComponentHandle&>(hComponent), out_pComponent);
  }

  template <typename ComponentType>
  [[nodiscard]] bool TryGetComponent(const plTypedComponentHandle<ComponentType>& hComponent, const ComponentType*& out_pComponent) const
  {
    return TryGetComponent<ComponentType>(static_cast<const plComponentHandle&>(hComponent), out_pComponent);
  }
