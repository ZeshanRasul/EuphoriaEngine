...
///
/// See struct \see plComponentHandle for more information about general component handle usage.
template <typename TYPE>
struct plTypedComponentHandle : public plComponentHandle
{
  plTypedComponentHandle() = default;
  explicit plTypedComponentHandle(const plComponentHandle& untyped)
  {
    m_InternalId = untyped.GetInternalID();
  }

  template <typename T, std::enable_if_t<std::is_convertible_v<T*, TYPE*>, bool> = true>
  explicit plTypedComponentHandle(const plTypedComponentHandle<T>& other)
    : plTypedComponentHandle(static_cast<const plComponentHandle&>(other))
  {
  }

  template <typename T, std::enable_if_t<std::is_convertible_v<T*, TYPE*>, bool> = true>
  PL_ALWAYS_INLINE void operator=(const plTypedComponentHandle<T>& other)
  {
    plComponentHandle::operator=(other);
  }
};
