...
PL_ALWAYS_INLINE plTypedComponentHandle<ComponentType> plComponentManagerBase::CreateComponent(plGameObject* pOwnerObject, ComponentType*& out_pComponent)
{
  plComponent* pComponent = nullptr;
  ...
  return plTypedComponentHandle<ComponentType>(hComponent);
}
...
{
  static_assert(PL_IS_DERIVED_FROM_STATIC(plComponent, ComponentType), "Not a valid component type");
}
