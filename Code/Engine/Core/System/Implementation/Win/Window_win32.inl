...
plResult plWindow::Initialize()
{
  ...
  if (m_CreationDescription.m_bSetForegroundOnInit && !plSystemInformation::IsDebuggerAttached())
  {
  ...
  // create window
  plStringWChar sTitleWChar(m_CreationDescription.m_Title.GetData());
  const wchar_t* sTitleWCharRaw = sTitleWChar.GetData();
  m_hWindowHandle = plMinWindows::FromNative(CreateWindowExW(dwExStyle, windowClass.lpszClassName, sTitleWCharRaw, dwWindowStyle, m_CreationDescription.m_Position.x, m_CreationDescription.m_Position.y, iWidth, iHeight, nullptr, nullptr, windowClass.hInstance, nullptr));
  ...
  if (m_CreationDescription.m_WindowMode == plWindowMode::WindowFixedResolution &&
      (m_CreationDescription.m_Resolution.width != plUInt32(r.right - r.left) ||
        m_CreationDescription.m_Resolution.height != plUInt32(r.bottom - r.top)))
  {
