...
void plStandardInputDevice::UpdateInputSlotValues()
{
  ...
  for (int i = 0; i < 5; ++i)
  {
    ...
    else
    {
      ...
      // Double clicking on a title bar maximizes a window but only fires a single mouse up event.
      // If that happens, no further clicks would be recognized because the balance between up and down events is broken.
      // So if the slot is not signaled and there is no down event but an up event instead, we just consume it.
...
void plStandardInputDevice::WindowMessage(plMinWindows::HWND hWnd, plMinWindows::UINT msg, plMinWindows::WPARAM wparam, plMinWindows::LPARAM lparam)
{
  ...
  if (m_bFirstWndMsg)
  {
    ...
    RECT r;
    GetWindowRect(plMinWindows::ToNative(hWnd), &r);
    SetCursorPos(plMath::Lerp(r.left, r.right, 0.5f), plMath::Lerp(r.bottom, r.top, 0.5f));
  }
