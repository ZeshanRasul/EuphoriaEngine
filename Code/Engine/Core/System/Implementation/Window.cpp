...
plResult plWindowCreationDesc::AdjustWindowSizeAndPosition()
{
  ...
  {
    m_Position.Set(pScreen->m_iOffsetX + (pScreen->m_iResolutionX - (plInt32)m_Resolution.width) / 2, pScreen->m_iOffsetY + (pScreen->m_iResolutionY - (plInt32)m_Resolution.height) / 2);
  }
  else
  {
...
void plWindowCreationDesc::SaveToDDL(plOpenDdlWriter& ref_writer)
{
  ...
  plOpenDdlUtils::StoreBool(ref_writer, m_bCenterWindowOnDisplay, "CenterWindowOnDisplay");
...
void plWindowCreationDesc::LoadFromDDL(const plOpenDdlReaderElement* pParentElement)
{
  if (const plOpenDdlReaderElement* pDesc = pParentElement->FindChildOfType("WindowDesc"))
  {
    ...
    if (const plOpenDdlReaderElement* pCenterWindowOnDisplay = pDesc->FindChildOfType(plOpenDdlPrimitiveType::Bool, "CenterWindowOnDisplay"))
...
{
  PL_IGNORE_UNUSED(hWnd);
  PL_IGNORE_UNUSED(msg);
  PL_IGNORE_UNUSED(wparam);
  PL_IGNORE_UNUSED(lparam);
}
#endif
