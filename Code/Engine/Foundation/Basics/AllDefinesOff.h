...
#define PL_ENABLED(x) (1 PL_PP_CONCAT(x, =) 1)

/// \brief Used in conjunction with PL_ON and PL_OFF for safe checks. Use #if PL_ENABLED(x) or #if PL_DISABLED(x) in conditional compilation.
#define PL_DISABLED(x) (1 PL_PP_CONCAT(x, =) 2)
...
#define PL_IS_NOT_EXCLUSIVE(x, y) ((1 PL_PP_CONCAT(x, =) 1) == (1 PL_PP_CONCAT(y, =) 1))

/// \brief Checks that exactly one of x, y and z is defined as PL_ON
#define PL_IS_NOT_EXCLUSIVE3(x, y, z) ((PL_ENABLED(x) + PL_ENABLED(y) + PL_ENABLED(z)) != 1)
...
#define PL_PLATFORM_ARCH_WEB PL_OFF
...
#define PL_USE_STRING_VALIDATION PL_OFF
...
#define PL_INTEROP_STL_SPAN PL_OFF
