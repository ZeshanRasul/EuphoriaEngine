...
#define PL_PP_CONCAT(x, y) PL_PP_CONCAT_HELPER(x, y)
#define PL_PP_CONCAT_HELPER(x, y) PL_PP_CONCAT_HELPER2(x, y)
#define PL_PP_CONCAT_HELPER2(x, y) x##y

/// \brief Concatenates two strings, even when the strings are macros themselves
#define PL_PP_CONCAT(x, y) PL_PP_CONCAT_HELPER(x, y)

/// \brief Turns some piece of code (usually some identifier name) into a string. Even works on macros.
#define PL_PP_STRINGIFY(str) PL_PP_STRINGIFY_HELPER(str)
#define PL_PP_STRINGIFY_HELPER(x) #x
