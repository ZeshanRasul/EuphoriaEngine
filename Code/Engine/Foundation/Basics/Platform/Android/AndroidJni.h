...
struct plJniError
{
  ...
  PL_ENUM_TO_STRING(SUCCESS, PENDING_EXCEPTION, NO_MATCHING_METHOD, AMBIGUOUS_CALL, NO_MATCHING_FIELD, CALL_ON_NULL_OBJECT, CLASS_NOT_FOUND);
};

using plJniErrorState = plEnum<plJniError>;
using plJniErrorHandler = plDelegate<void(plJniErrorState)>;
...
///   \endcode
///
/// Note that all JNICalls may fail, including the construction of plJniClass and plJniObject instances.
...
///       plJniObject activity = attachment.GetActivity();
///       plJniClass myClassType = activity.Call<plJniObject>("getClassLoader").Call<plJniClass>("loadClass", plJniString("com.myproject.MyClass"));
///       if(attachment.GetLastError() == plJniErrorState::NO_MATCHING_METHOD){SpecificErrorHandling();}
///     }
///   \endcode
///
...
///         attachment
///           .GetActivity()
///           .Call<plJniObject>("getClassLoader")
///           .Call<plJniClass>("loadClass", plJniString("com.myproject.MyClass")
///           .CreateInstance()
///           .Call<plJniObject>("MyMethod");
///       }
...
///   \endcode
///
/// Either way, the error is always internally logged with plLog::Error.
/// ClearLastError() is called whenever a plJniAttachment is destroyed, so you needn't worry about cleanup in that case.
...
public:
  ...
  /// \brief Installs an error handler valid for the current plJniAttachment's lifetime. Called for all plJniErrorStates but SUCCESS.
  /// There must not be any other plJniAttachments existing on the same thread for the call to succeed.
  /// There must not be any new plJniAttachments instances created before the current plJniAttachment is destroyed.
  ...
  void InstallErrorHandler(plJniErrorHandler onError);
...
/// \brief Represents the null value of an plJniClass.
/// Passing null / nullptr directly to plJni results in type information being lost,
/// without which plJni can't make the appropriate JNI calls.
/// create an plJniClass instance instead, and wrap it in an plJniNullPtr instance.
/// You may then pass that plJniNullPtr instance to any plJni calls you make.
class PL_FOUNDATION_DLL plJniNullPtr
{
  plJniClass m_class;

public:
  /// \brief Constructs a Class from a plJniClass.
  ///
  /// \param clazz The plJniClass.
  explicit plJniNullPtr(plJniClass& clazz);

  /// \brief Returns the fully qualified name of the plJniClass that was passed into the constructor, e.g. "java/lang/String"
  const plJniString GetTypeSignature() const;
};
