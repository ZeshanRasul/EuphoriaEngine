...
thread_local plJniErrorHandler s_onError;
...
plJniAttachment::plJniAttachment()
{
  ...
  else
  {
    ...
    if (ownsEnv)
    {
      const char* szThreadName = "PL JNI";
      if (const plThread* pThread = plThread::GetCurrentThread())
      {
        szThreadName = pThread->GetThreadName();
      }
      else if (plThreadUtils::IsMainThread())
      {
        szThreadName = "PL Main Thread";
      }
  ...
  s_attachCount++;
  PL_ASSERT_ALWAYS(s_onError.IsValid() == false, "Can't install error handler for more than one instance.");
}
...
{
  s_lastError = state;
  if (s_onError.IsValid() && s_lastError != plJniErrorState::SUCCESS)
  {
...
void plJniAttachment::InstallErrorHandler(plJniErrorHandler onError)
{
  PL_ASSERT_ALWAYS(s_attachCount == 1, "Can't install error handler for more than one instance.");
  s_onError = onError;
}
...
plJniNullPtr::plJniNullPtr(plJniClass& clazz)
{
  m_class = clazz;
}

const plJniString plJniNullPtr::GetTypeSignature() const
{
  plJniString jSignature = m_class.UnsafeCall<plJniString>("getName", "()Ljava/lang/String;");
  plStringBuilder signature{jSignature.GetData()};
  signature.ReplaceAll(".", "/");
  return plJniString{signature.GetData()};
}
