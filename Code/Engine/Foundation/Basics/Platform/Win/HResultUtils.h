...
#define PL_HRESULT_TO_FAILURE_LOG(code)                                                         \
  ...
  {                                                                                             \
    ...
    if (s < 0)                                                                                  \
    {                                                                                           \
      plLog::Error("Call '{0}' failed with: {1}", PL_PP_STRINGIFY(code), plHRESULTtoString(s)); \
      return PL_FAILURE;                                                                        \
    }                                                                                           \
  } while (false)
...
#define PL_HRESULT_TO_ASSERT(code)                                                                     \
  ...
  {                                                                                                    \
    plMinWindows::HRESULT s = (code);                                                                  \
    PL_ASSERT_DEV(s >= 0, "Call '{0}' failed with: {1}", PL_PP_STRINGIFY(code), plHRESULTtoString(s)); \
  } while (false)
