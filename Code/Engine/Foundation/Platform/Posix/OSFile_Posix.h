...
#if PL_DISABLED(PL_PLATFORM_WINDOWS_UWP)  && !defined(PL_SKIP_FOLDER_PATHS)
...
#  elif PL_ENABLED(PL_PLATFORM_ANDROID)
    {
      ...
      sTemp.AppendPath("Assets/plDummyBin");
      s_sApplicationPath = sTemp;
    }
#  else
    char result[PATH_MAX];
    ssize_t length = readlink("/proc/self/exe", result, PATH_MAX);
    s_sApplicationPath = plStringView(result, result + length);
#  endif
  }
