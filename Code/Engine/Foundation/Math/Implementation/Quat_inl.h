...
template <typename Type>
PL_ALWAYS_INLINE plVec3Template<Type> plQuatTemplate<Type>::Rotate(const plVec3Template<Type>& v) const
{
  return *this * v;
}
...
template <typename Type>
plQuatTemplate<Type> plQuatTemplate<Type>::MakeRotationTowardsTarget(const plVec3Template<Type>& vTargetDir, const plQuatTemplate<Type>& vCurrentRot, const float fMaxAngularSpeed, const plVec3Template<Type>& vAxis)
{
  plAngle currentAngle = 2.0 * plMath::ACos(vCurrentRot.w);
  if (vCurrentRot.z < 0)
  ...
  if (fAngleDiff > plMath::Pi<float>())
    ...
    fAngleDiff += 2.0f * plMath::Pi<float>();
  fAngleDiff = plMath::Sign(fAngleDiff) * plMath::Min(plMath::Abs(fAngleDiff), fMaxAngularSpeed);

  return plQuat::MakeFromAxisAndAngle(vAxis, plAngle::MakeFromRadian(currentAngle.GetRadian() + fAngleDiff));
}
