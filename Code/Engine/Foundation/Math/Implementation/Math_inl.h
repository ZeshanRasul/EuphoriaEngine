...
PL_ALWAYS_INLINE constexpr plUInt32 plMath::WrapUInt(plUInt32 uiValue, plUInt32 uiExcludedMaxValue)
{
  return uiValue % uiExcludedMaxValue;
}

PL_ALWAYS_INLINE constexpr plInt32 plMath::WrapInt(plInt32 iValue, plUInt32 uiExcludedMaxValue)
{
  const plInt32 wrapped = (iValue % static_cast<plInt32>(uiExcludedMaxValue));
  return wrapped >= 0 ? wrapped : (wrapped + uiExcludedMaxValue);
}

PL_ALWAYS_INLINE constexpr plInt32 plMath::WrapInt(plInt32 iValue, plInt32 iMinValue, plInt32 iExcludedMaxValue)
{
  PL_ASSERT_DEBUG(iMinValue < iExcludedMaxValue, "Invalid range to wrap integer around.");
  return iMinValue + WrapInt(iValue - iMinValue, static_cast<plUInt32>(iExcludedMaxValue - iMinValue));
}

PL_ALWAYS_INLINE float plMath::WrapFloat01(float fValue)
{
  if (fValue < 0.0f)
  {
...
PL_ALWAYS_INLINE float plMath::WrapFloat(float fValue, float fMinValue, float fMaxValue)
{
