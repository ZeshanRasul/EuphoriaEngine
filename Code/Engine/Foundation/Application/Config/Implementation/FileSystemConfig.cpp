...
void plApplicationFileSystemConfig::Apply()
{
  ...
  for (const auto& var : m_DataDirs)
  {
    ...
    {
      plFileSystem::AddDataDirectory(var.m_sDataDirSpecialPath, "AppFileSystemConfig", var.m_sRootName, (!var.m_sRootName.IsEmpty() && var.m_bWritable) ? plDataDirUsage::AllowWrites : plDataDirUsage::ReadOnly).IgnoreResult();
    }
  }
}
