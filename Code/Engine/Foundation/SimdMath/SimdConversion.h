...
namespace plSimdConversion
{
  ...
  PL_ALWAYS_INLINE plVec3I32 ToVec3i(const plSimdVec4i& v)
  {
    plVec4I32 tmp;
    v.Store<4>(&tmp.x);
    return *reinterpret_cast<plVec3I32*>(&tmp.x);
  }

  PL_ALWAYS_INLINE plSimdVec4i ToVec3i(const plVec3I32& v)
  {
    plSimdVec4i tmp;
    tmp.Load<3>(&v.x);
    return tmp;
  ...
  PL_ALWAYS_INLINE plVec4I32 ToVec4i(const plSimdVec4i& v)
  {
    plVec4I32 tmp;
    v.Store<4>(&tmp.x);
    return tmp;
  ...
  PL_ALWAYS_INLINE plSimdVec4i ToVec4i(const plVec4I32& v)
  {
    plSimdVec4i tmp;
    tmp.Load<4>(&v.x);
    return tmp;
  ...
  PL_ALWAYS_INLINE plSimdBSphere ToBSphere(const plBoundingSphere& s)
  {
    return plSimdBSphere(ToVec3(s.m_vCenter), s.m_fRadius);
  }

  PL_ALWAYS_INLINE plSimdBBox ToBBox(const plBoundingBox& b)
  {
    return plSimdBBox(ToVec3(b.m_vMin), ToVec3(b.m_vMax));
  }

  PL_ALWAYS_INLINE plBoundingBox ToBBox(const plSimdBBox& b)
  {
    return plBoundingBox::MakeFromMinMax(ToVec3(b.m_Min), ToVec3(b.m_Max));
  }
