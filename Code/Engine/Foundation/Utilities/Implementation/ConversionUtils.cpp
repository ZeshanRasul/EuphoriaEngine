...
#define Check(name)                                     \
  if (sColorName.IsEqual_NoCase(PL_PP_STRINGIFY(name))) \
  return plColor::name

  plResult ConvertHexStringToColor(plStringView sText, plColorGammaUB& ref_color)
  {
    ref_color = plColorGammaUB(0, 0, 0);

    auto twoCharsToByte = [](plStringView& text, plUInt8& out_uiByte) -> plResult
    {
      if (text.IsEmpty())
        return PL_SUCCESS;
    ...
    PL_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.r));
    PL_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.g));
    PL_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.b));
    PL_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.a));
