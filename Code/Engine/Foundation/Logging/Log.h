...
/// analysis happy.
#define PL_LOG_BLOCK plLogBlock PL_PP_CONCAT(_logblock_, PL_SOURCE_LINE)

/// \brief Use this helper macro to easily mute all logging in a scope.
#define PL_LOG_BLOCK_MUTE()                               \
  plMuteLog PL_PP_CONCAT(_logmuteblock_, PL_SOURCE_LINE); \
  plLogSystemScope PL_PP_CONCAT(_logscope_, PL_SOURCE_LINE)(&PL_PP_CONCAT(_logmuteblock_, PL_SOURCE_LINE))
