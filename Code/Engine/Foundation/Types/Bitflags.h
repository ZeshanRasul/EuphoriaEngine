...
public:
  ...
  PL_ALWAYS_INLINE ConstIterator GetIterator() const // [tested]
  {
    return ConstIterator((Enum)m_Value);
  }

  /// \brief Returns an invalid iterator. Needed to support range based for loops.
  PL_ALWAYS_INLINE ConstIterator GetEndIterator() const // [tested]
  {
...
template <typename T>
typename plBitflags<T>::ConstIterator begin(const plBitflags<T>& container)
{
  return container.GetIterator();
}

template <typename T>
typename plBitflags<T>::ConstIterator cbegin(const plBitflags<T>& container)
{
  return container.GetIterator();
}

template <typename T>
typename plBitflags<T>::ConstIterator end(const plBitflags<T>& container)
{
  return container.GetEndIterator();
}

template <typename T>
typename plBitflags<T>::ConstIterator cend(const plBitflags<T>& container)
{
  return container.GetEndIterator();
}
