...
PL_FORCE_INLINE bool plVariant::operator==(const T& other) const
{
  ...
  else if constexpr (std::is_same_v<T, plStringView>)
  {
    if (m_uiType == Type::String)
    {
      return Cast<plString>().GetView() == other;
    }
  }
  else if constexpr (std::is_same_v<T, plString>)
  {
    ...
    {
      return Cast<plStringView>() == other.GetView();
    }
  }
...
PL_FORCE_INLINE void plVariant::InitInplace(const T& value)
{
  ...
  static_assert(plGetTypeClass<T>::value <= plTypeIsMemRelocatable::value, "in place data needs to be POD or mem relocatable");
...
PL_FORCE_INLINE void plVariant::InitTypedObject(const T& value, plTraitInt<1>)
{
  ...
  static_assert(TypeDeduction<T>::value == Type::TypedObject, "value of this type cannot be stored in a Variant");
  static_assert(plIsPodType<T>::value, "in place data needs to be POD");
  plMemoryUtils::CopyConstruct(reinterpret_cast<T*>(&m_Data), value, 1);
