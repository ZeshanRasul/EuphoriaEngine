...
public:
  ...
  plResult WriteToLeafObject(void* pRootObject, const plRTTI* pType, plDelegate<void(void* pLeaf, const plRTTI& pType)> func) const;
  ///\brief Applies the entire path and allows reading from the target object.
  plResult ReadFromLeafObject(void* pRootObject, const plRTTI* pType, plDelegate<void(void* pLeaf, const plRTTI& pType)> func) const;
