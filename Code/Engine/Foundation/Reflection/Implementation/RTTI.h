...
public:
  ...
  struct ForEachOptions
  {
    ...
    {
      None = 0,
      ExcludeNonAllocatable = PL_BIT(0), ///< Excludes all types that cannot be allocated through plRTTI. They may still be creatable through regular C++, though.
      ExcludeAbstract = PL_BIT(1),       ///< Excludes all types that are marked as 'abstract'. They may not be abstract in the C++ sense, though.
