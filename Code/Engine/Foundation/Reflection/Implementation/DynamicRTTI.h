...
/// \brief Same as PL_BEGIN_DYNAMIC_REFLECTED_TYPE but forces the type to be treated as abstract by reflection even though
/// it might not be abstract from a C++ perspective.
#define PL_BEGIN_ABSTRACT_DYNAMIC_REFLECTED_TYPE(Type, Version)     \
  PL_BEGIN_DYNAMIC_REFLECTED_TYPE(Type, Version, plRTTINoAllocator) \
    flags.Add(plTypeFlags::Abstract);

#define PL_END_ABSTRACT_DYNAMIC_REFLECTED_TYPE PL_END_DYNAMIC_REFLECTED_TYPE
