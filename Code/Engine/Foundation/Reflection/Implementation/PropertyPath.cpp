...
plResult plPropertyPath::WriteToLeafObject(void* pRootObject, const plRTTI* pType, plDelegate<void(void* pLeaf, const plRTTI& pType)> func) const
{
  PL_ASSERT_DEBUG(
...
plResult plPropertyPath::ReadFromLeafObject(void* pRootObject, const plRTTI* pType, plDelegate<void(void* pLeaf, const plRTTI& pType)> func) const
{
  PL_ASSERT_DEBUG(
    m_PathSteps.IsEmpty() || m_PathSteps[m_PathSteps.GetCount() - 1].m_pProperty->GetSpecificType()->GetTypeFlags().IsSet(plTypeFlags::Class),
    "To resolve the leaf object the path needs to be empty or end in a class.");
