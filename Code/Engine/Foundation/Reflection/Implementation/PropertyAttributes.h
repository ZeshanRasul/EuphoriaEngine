...
class PL_FOUNDATION_DLL plIconAttribute : public plPropertyAttribute
{
  PL_ADD_DYNAMIC_REFLECTION(plIconAttribute, plPropertyAttribute);

public:
  plIconAttribute() = default;
  plIconAttribute(const char* szIcon)
    : m_sIcon(szIcon)
  {
  }

  const char* GetIcon() const { return m_sIcon; }

private:
  plUntrackedString m_sIcon;
};
...
private:
  plUntrackedString m_sTypeFilter;
  plUntrackedString m_sRequiredTag;
  plBitflags<plDependencyFlags> m_DependencyFlags;
};
...
class PL_FOUNDATION_DLL plScriptableFunctionAttribute : public plPropertyAttribute
{
  ...
  plUInt32 GetArgumentCount() const { return m_ArgNames.GetCount(); }
