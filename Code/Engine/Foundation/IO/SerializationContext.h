...
#define PL_IMPLEMENT_SERIALIZATION_CONTEXT(type)                                                                                        \
  thread_local type* PL_PP_CONCAT(s_pActiveContext, type);                                                                              \
  type* type::GetContext()                                                                                                              \
  {                                                                                                                                     \
    return PL_PP_CONCAT(s_pActiveContext, type);                                                                                        \
  }                                                                                                                                     \
  void type::SetContext(plSerializationContext* pContext)                                                                               \
  {                                                                                                                                     \
    PL_ASSERT_DEV(pContext == nullptr || PL_PP_CONCAT(s_pActiveContext, type) == nullptr, "Only one context can be active at a time."); \
    PL_PP_CONCAT(s_pActiveContext, type) = static_cast<type*>(pContext);                                                                \
  }
