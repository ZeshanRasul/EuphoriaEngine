...
{
  // This assert helps finding cases where the plFileSystem is used without pl being properly initialized or already shutdown.
...
plResult plFileSystem::AddDataDirectory(plStringView sDataDirectory, plStringView sGroup, plStringView sRootName, plDataDirUsage usage)
{
  PL_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
  PL_ASSERT_DEV(usage != plDataDirUsage::AllowWrites || !sRootName.IsEmpty(), "A data directory must have a non-empty, unique name to be mounted for write access");
...
const plDataDirectoryInfo* plFileSystem::FindDataDirectoryWithRoot(plStringView sRootName)
{
...
const plDataDirectoryInfo& plFileSystem::GetDataDirectoryInfo(plUInt32 uiDataDirIndex)
{
  PL_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
...
plStringView plFileSystem::GetDataDirRelativePath(plStringView sPath, plUInt32 uiDataDir)
{
  ...
  const plString128& sRedDirPath = s_pData->m_DataDirectories[uiDataDir].m_pDataDirType->GetRedirectedDataDirectoryPath();
  ...
  const plString128& sDirPath = s_pData->m_DataDirectories[uiDataDir].m_pDataDirType->GetDataDirectoryPath();
...
plDataDirectoryInfo* plFileSystem::GetDataDirForRoot(const plString& sRoot)
{
  PL_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
  PL_LOCK(s_pData->m_FsMutex);
...
void plFileSystem::DeleteFile(plStringView sFile)
{
  ...
  for (plInt32 i = (plInt32)s_pData->m_DataDirectories.GetCount() - 1; i >= 0; --i)
  {
    ...
    if (s_pData->m_DataDirectories[i].m_Usage != plDataDirUsage::AllowWrites)
      continue;
...
{
  plStringView root, path;
  plPathUtils::GetRootedPathParts(sPath, root, path);

  plStringBuilder rootUpr = root;
  rootUpr.ToUpper();
  rootName = rootUpr;
  return path;
}
...
plDataDirectoryReader* plFileSystem::GetFileReader(plStringView sFile, plFileShareMode::Enum FileShareMode, bool bAllowFileEvents)
{
  ...
  for (plInt32 i = (plInt32)s_pData->m_DataDirectories.GetCount() - 1; i >= 0; --i)
  {
    ...
    plDataDirectoryReader* pReader = s_pData->m_DataDirectories[i].m_pDataDirType->OpenFileToRead(sRelPath, FileShareMode, bOneSpecificDataDir);
...
plDataDirectoryWriter* plFileSystem::GetFileWriter(plStringView sFile, plFileShareMode::Enum FileShareMode, bool bAllowFileEvents)
{
  ...
  for (plInt32 i = (plInt32)s_pData->m_DataDirectories.GetCount() - 1; i >= 0; --i)
  {
    ...
    plDataDirectoryWriter* pWriter = s_pData->m_DataDirectories[i].m_pDataDirType->OpenFileToWrite(sRelPath, FileShareMode);
...
plResult plFileSystem::ResolvePath(plStringView sPath, plStringBuilder* out_pAbsolutePath, plStringBuilder* out_pDataDirRelativePath, const plDataDirectoryInfo** out_pDataDir /*= nullptr*/)
{
  PL_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
  ...
  if (sPath.StartsWith(":"))
  {
    ...
    const plDataDirectoryInfo* pDataDir = GetDataDirForRoot(sRootName);
  ...
  else if (plPathUtils::IsAbsolutePath(sPath))
  {
    ...
    for (plUInt32 dd = s_pData->m_DataDirectories.GetCount(); dd > 0; --dd)
    {
      ...
      if (plPathUtils::IsSubPath(dir.m_pDataDirType->GetRedirectedDataDirectoryPath(), absPath))
      {
...
plResult plFileSystem::DetectSdkRootDirectory(plStringView sExpectedSubFolder /*= "Data/Base"*/)
{
  ...
  PL_IGNORE_UNUSED(sExpectedSubFolder);
...
void plFileSystem::StartSearch(plFileSystemIterator& ref_iterator, plStringView sSearchTerm, plBitflags<plFileSystemIteratorFlags> flags /*= plFileSystemIteratorFlags::Default*/)
{
  ...
  plStringBuilder sDdPath, sRelPath;

  if (sSearchTerm.IsRootedPath())
  {
    const plStringView root = sSearchTerm.GetRootedPathRootName();

    const plDataDirectoryInfo* pDataDir = FindDataDirectoryWithRoot(root);
    if (pDataDir == nullptr)
      return;
  ...
  {
    for (plUInt32 idx = s_pData->m_DataDirectories.GetCount(); idx > 0; --idx)
    {
...
plResult plFileSystem::CreateDirectoryStructure(plStringView sPath)
{
  ...
  {
    plFileSystem::ResolvePath(sRedir, &sRedir, nullptr).AssertSuccess();
  }
