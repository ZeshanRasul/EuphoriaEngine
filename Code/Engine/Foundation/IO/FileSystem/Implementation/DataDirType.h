...
struct plFileStats;
class plDataDirectoryType;

/// \brief Describes in which mode a data directory is mounted.
enum class plDataDirUsage
{
  ReadOnly,
  AllowWrites,
};

struct plDataDirectoryInfo
{
  plDataDirUsage m_Usage;

  plString m_sRootName;
  plString m_sGroup;
  plDataDirectoryType* m_pDataDirType = nullptr;
};
...
protected:
  ...
  virtual void DeleteFile(plStringView sFile) { PL_IGNORE_UNUSED(sFile); }
  ...
  {
    PL_IGNORE_UNUSED(sPathOrAssetGuid);
    PL_IGNORE_UNUSED(out_sRedirection);
    return false;
  }
...
protected:
  ...
  virtual void OnReaderWriterClose(plDataDirectoryReaderWriterBase* pClosed) { PL_IGNORE_UNUSED(pClosed); }
...
public:
  ...
  virtual plUInt64 Skip(plUInt64 uiBytes)
  {
    ...
    {
      plUInt64 uiBytesToRead = plMath::Min<plUInt64>(uiBytes - uiBytesSkipped, 1024);

      plUInt64 uiBytesRead = Read(uiTempBuffer, uiBytesToRead);
