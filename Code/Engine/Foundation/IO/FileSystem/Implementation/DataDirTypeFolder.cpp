...
namespace plDataDirectory
{
  ...
  plUInt64 FolderReader::Skip(plUInt64 uiBytes)
  {
    if (uiBytes == 0)
    {
      return 0;
    ...
    const plUInt64 fileSize = m_File.GetFileSize();
    const plUInt64 origFilePosition = m_File.GetFilePosition();
    PL_ASSERT_DEBUG(origFilePosition <= fileSize, "");

    const plUInt64 newFilePosition = plMath::Min(fileSize, origFilePosition + uiBytes);
    m_File.SetFilePosition(newFilePosition, plFileSeekMode::FromStart);
    PL_ASSERT_DEBUG(newFilePosition == m_File.GetFilePosition(), "");

    PL_ASSERT_DEBUG(newFilePosition >= origFilePosition, "");
  ...
  plDataDirectoryType* FolderType::Factory(plStringView sDataDirectory, plStringView sGroup, plStringView sRootName, plDataDirUsage usage)
  {
