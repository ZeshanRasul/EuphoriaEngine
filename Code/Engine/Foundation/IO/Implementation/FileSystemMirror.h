...
public:
  ...
  plResult GetType(plStringView sPath, Type& out_type);
...
template <typename T>
plResult plFileSystemMirror<T>::GetType(plStringView sPath0, Type& out_type)
{
  plStringBuilder sPath = sPath0;
  DirEntry* dir = FindDirectory(sPath);
  if (dir == nullptr)
  ...
  if (it.IsValid())
  {
    out_type = plFileSystemMirror::Type::File;
    return PL_SUCCESS;
  }
  ...
  if (itDir.IsValid())
  {
    out_type = plFileSystemMirror::Type::Directory;
    return PL_SUCCESS;
  }
