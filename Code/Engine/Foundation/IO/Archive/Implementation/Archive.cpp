...
plUInt32 plArchiveTOC::AddPathString(plStringView sPathString)
{
  const plUInt32 offset = m_AllPathStrings.GetCount();
  const plUInt32 numNewBytesNeeded = sPathString.GetElementCount() + 1;
  m_AllPathStrings.Reserve(m_AllPathStrings.GetCount() + numNewBytesNeeded);
  m_AllPathStrings.PushBackRange(plArrayPtr<const plUInt8>(reinterpret_cast<const plUInt8*>(sPathString.GetStartPointer()), sPathString.GetElementCount()));
  m_AllPathStrings.PushBackUnchecked('\0');
  return offset;
}

void plArchiveTOC::RebuildPathToEntryHashes()
{
