...
plDataDirectoryType* plDataDirectory::ArchiveType::Factory(plStringView sDataDirectory, plStringView sGroup, plStringView sRootName, plDataDirUsage usage)
{
  PL_IGNORE_UNUSED(sGroup);
  PL_IGNORE_UNUSED(sRootName);
  PL_IGNORE_UNUSED(usage);
...
{
  PL_IGNORE_UNUSED(bSpecificallyThisDataDir);
...

  {
    ...
    switch (pEntry->m_CompressionMode)
    {
      ...
      case plArchiveCompressionMode::Compressed_zip:
      {
        ...
        {
          m_ReadersZip.PushBack(PL_DEFAULT_NEW(ArchiveReaderZip, 2));
...
{
  PL_IGNORE_UNUSED(bOneSpecificDataDir);
...
plDataDirectory::ArchiveReaderCommon::ArchiveReaderCommon(plInt32 iDataDirUserData)
  : plDataDirectoryReader(iDataDirUserData)
{
}

plUInt64 plDataDirectory::ArchiveReaderCommon::GetFileSize() const
{
...
plUInt64 plDataDirectory::ArchiveReaderUncompressed::Skip(plUInt64 uiBytes)
{
...
{
  PL_IGNORE_UNUSED(FileShareMode);
...
void plDataDirectory::ArchiveReaderZstd::InternalClose()
{
  // nothing to do
}
#endif
...
plDataDirectory::ArchiveReaderZip::ArchiveReaderZip(plInt32 iDataDirUserData)
  : ArchiveReaderUncompressed(iDataDirUserData)
{
}

plDataDirectory::ArchiveReaderZip::~ArchiveReaderZip() = default;

plUInt64 plDataDirectory::ArchiveReaderZip::Read(void* pBuffer, plUInt64 uiBytes)
{
  return m_CompressedStreamReader.ReadBytes(pBuffer, uiBytes);
}

plResult plDataDirectory::ArchiveReaderZip::InternalOpen(plFileShareMode::Enum FileShareMode)
{
  PL_IGNORE_UNUSED(FileShareMode);
  PL_ASSERT_DEBUG(FileShareMode != plFileShareMode::Exclusive, "Archives only support shared reading of files. Exclusive access cannot be guaranteed.");
