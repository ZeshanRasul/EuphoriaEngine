...
plResult plArchiveReader::OpenArchive(plStringView sPath)
{
  ...
  // validate the archive
  {
    ...
    if (plArchiveUtils::IsAcceptedArchiveFileExtensions(extension))
    {
      ...
      m_pDataStart = m_MemFile.GetReadPointer(plArchiveUtils::ArchiveHeaderSize, plMemoryMappedFile::OffsetBase::Start);
...
#  ifdef BUILDSYSTEM_ENABLE_ZLIB_SUPPORT
    ...
    {
      PL_SUCCEED_OR_RETURN(plArchiveUtils::ReadZipHeader(reader, m_uiArchiveVersion));
      if (m_uiArchiveVersion != 0)
      {
        plLog::Error("Unknown zip version '{}'", m_uiArchiveVersion);
        return PL_FAILURE;
      }
      m_pDataStart = m_MemFile.GetReadPointer(0, plMemoryMappedFile::OffsetBase::Start);

      if (plArchiveUtils::ExtractZipTOC(m_MemFile, m_ArchiveTOC).Failed())
      {
        plLog::Error("Failed to deserialize zip TOC");
        return PL_FAILURE;
      }
    }
#  endif
