...
plResult plProfilingUtils::SaveProfilingCapture(plStringView sCapturePath)
{
  ...
  if (fileWriter.Open(sCapturePath) == PL_SUCCESS)
  {
    ...
    // Set sort index to plInvalidIndex so that the runtime process is always at the bottom and editor is always on top when opening the trace.
    profilingData.m_uiProcessSortIndex = plInvalidIndex;
