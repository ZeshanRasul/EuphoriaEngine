...
template <typename DerivedLhs, typename DerivedRhs>
PL_ALWAYS_INLINE std::strong_ordering operator<=>(const plStringBase<DerivedLhs>& lhs, const plStringBase<DerivedRhs>& rhs)
{
  return lhs.Compare(rhs) <=> 0;
}

template <typename DerivedLhs, typename DerivedRhs>
PL_ALWAYS_INLINE std::strong_ordering operator<=>(const plStringBase<DerivedLhs>& lhs, const char* rhs)
{
  return lhs.Compare(rhs) <=> 0;
}
...
template <typename Derived>
plStringView plStringBase<Derived>::GetFileExtension(bool bFullExtension) const
{
