...
template <typename T>
constexpr PL_ALWAYS_INLINE plStringView::plStringView(const T&& str, typename std::enable_if<std::is_same<T, const char*>::value == false && std::is_convertible<T, const char*>::value, int>::type*)
{
  m_pStart = str;
  m_uiElementCount = plStringUtils::GetStringElementCount(m_pStart);
}

constexpr PL_ALWAYS_INLINE plStringView::plStringView(const char* pStart, const char* pEnd)
{
  PL_ASSERT_DEBUG(pStart <= pEnd, "Invalid pointers to construct a string view from.");
...
template <size_t N>
constexpr PL_ALWAYS_INLINE plStringView::plStringView(const char (&str)[N])
  : m_pStart(str)
...
template <size_t N>
constexpr PL_ALWAYS_INLINE plStringView::plStringView(char (&str)[N])
{
  m_pStart = str;
...
PL_ALWAYS_INLINE std::strong_ordering operator<=>(plStringView lhs, plStringView rhs)
{
