...
class PL_FOUNDATION_DLL plStringView : public plThisIsAString
{
public:
  PL_DECLARE_POD_TYPE();
  ...
  template <typename T>
  constexpr PL_ALWAYS_INLINE plStringView(const T&& str, typename std::enable_if<std::is_same<T, const char*>::value == false && std::is_convertible<T, const char*>::value, int>::type* = 0); // [tested]

  /// \brief Creates a string view for the range from pStart to pEnd.
  constexpr plStringView(const char* pStart, const char* pEnd); // [tested]
  ...
  plStringView GetFileExtension(bool bFullExtension = false) const; // [tested]
