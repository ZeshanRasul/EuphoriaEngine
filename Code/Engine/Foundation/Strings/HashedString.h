...
public:
  ...
  /// The string hash may not be known, if the value was never assigned to any plHashedString, in which case PL_FAILURE is returned.
  static plResult LookupStringHash(plUInt64 uiHash, plStringView& out_sResult);
...
public:
  ...
  /// \brief Convenience function to call plHashedString::LookupStringHash().
  plResult LookupStringHash(plStringView& out_sResult) const
  {
    return plHashedString::LookupStringHash(m_uiHash, out_sResult);
  }
