...
#if PL_ENABLED(PL_COMPILE_FOR_DEVELOPMENT) && TRACY_ENABLE && TRACY_ENABLE_MEMORY_TRACKING
#  include <tracy/tracy/Tracy.hpp>

#  define PL_TRACY_CALLSTACK_DEPTH 16
#  define PL_TRACY_ALLOC_CS(ptr, size, name) TracyAllocNS(ptr, size, PL_TRACY_CALLSTACK_DEPTH, name)
#  define PL_TRACY_FREE_CS(ptr, name) TracyFreeNS(ptr, PL_TRACY_CALLSTACK_DEPTH, name)
#  define PL_TRACY_ALLOC(ptr, size, name) TracyAllocN(ptr, size, name)
#  define PL_TRACY_FREE(ptr, name) TracyFreeN(ptr, name)
#else
#  define PL_TRACY_ALLOC_CS(ptr, size, name)
#  define PL_TRACY_FREE_CS(ptr, name)
#  define PL_TRACY_ALLOC(ptr, size, name)
#  define PL_TRACY_FREE(ptr, name)
#endif
...
void plMemoryTracker::DeregisterAllocator(plAllocatorId allocatorId)
{
  ...
  if (uiLiveAllocations != 0 && data.m_TrackingMode > plAllocatorTrackingMode::AllocationStatsIgnoreLeaks)
  {
...
void plMemoryTracker::AddAllocation(plAllocatorId allocatorId, plAllocatorTrackingMode mode, const void* pPtr, size_t uiSize, size_t uiAlign, plTime allocationTime)
{
...

  {
    ...
    if (mode >= plAllocatorTrackingMode::AllocationStatsAndStacktraces)
    {
      PL_TRACY_ALLOC_CS(pPtr, uiSize, data.m_sName.GetData());
    }
    else
    {
      PL_TRACY_ALLOC(pPtr, uiSize, data.m_sName.GetData());
    }
  }
}
...
void plMemoryTracker::RemoveAllocation(plAllocatorId allocatorId, const void* pPtr)
{
...

  {
    ...
    if (data.m_Allocations.Remove(pPtr, &info))
    {
      ...
      if (data.m_TrackingMode >= plAllocatorTrackingMode::AllocationStatsAndStacktraces)
      {
        PL_TRACY_FREE_CS(pPtr, data.m_sName.GetData());
      }
      else
      {
        PL_TRACY_FREE(pPtr, data.m_sName.GetData());
      }
    }
    else
    {
...
void plMemoryTracker::RemoveAllAllocations(plAllocatorId allocatorId)
{
  ...
  for (auto it = data.m_Allocations.GetIterator(); it.IsValid(); ++it)
  {
    ...
    if (data.m_TrackingMode >= plAllocatorTrackingMode::AllocationStatsAndStacktraces)
    {
      ...
      {
        PL_IGNORE_UNUSED(alloc);
        PL_TRACY_FREE_CS(alloc.Key(), data.m_sName.GetData());
      }
    }
    else
    {
      ...
      {
        PL_IGNORE_UNUSED(alloc);
        PL_TRACY_FREE(alloc.Key(), data.m_sName.GetData());
      }
    }
