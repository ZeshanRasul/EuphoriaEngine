...
public:
  ...
  void FlipBit(plUInt32 uiBit); // [tested]
  ...
  void FlipBitRange(plUInt32 uiFirstBit, plUInt32 uiNumBits); // [tested]

  /// \brief Swaps two bitfields
  void Swap(plBitfield<Container>& other); // [tested]
  struct ConstIterator
  {
    ...
    PL_FORCE_INLINE ConstIterator() = default; // [tested]

    // Start iterator.
    explicit ConstIterator(const plBitfield<Container>& bitfield); // [tested]

    /// \brief Checks whether this iterator points to a valid element.
    bool IsValid() const; // [tested]

    /// \brief Returns the 'value' of the element that this iterator points to.
    plUInt32 Value() const; // [tested]
    ...
    plUInt32 operator*() const; // [tested]

    /// \brief Shorthand for 'Next'.
    void operator++(); // [tested]

  private:
    void FindNextChunk(plUInt32 uiStartChunk);

  private:
    plUInt32 m_uiChunk = 0;
    sub_iterator m_Iterator;
    const plBitfield<Container>* m_pBitfield = nullptr;
  };
...
typename plBitfield<Container>::ConstIterator begin(const plBitfield<Container>& container)
{
  return container.GetIterator();
}

template <typename Container>
typename plBitfield<Container>::ConstIterator cbegin(const plBitfield<Container>& container)
{
  return container.GetIterator();
}

template <typename Container>
typename plBitfield<Container>::ConstIterator end(const plBitfield<Container>& container)
{
  return container.GetEndIterator();
}

template <typename Container>
typename plBitfield<Container>::ConstIterator cend(const plBitfield<Container>& container)
{
  return container.GetEndIterator();
}
...
public:
  using StorageType = T;
  using ConstIterator = plBitIterator<StorageType, true, plUInt32>;
  ...
  /// \brief Swaps two bitfields
  void Swap(plStaticBitfield<T>& other); // [tested]
...
// begin() /end() for range-based for-loop support
template <typename Container>
typename plStaticBitfield<Container>::ConstIterator begin(const plStaticBitfield<Container>& container)
{
  return container.GetIterator();
}

template <typename Container>
typename plStaticBitfield<Container>::ConstIterator cbegin(const plStaticBitfield<Container>& container)
{
  return container.GetIterator();
}

template <typename Container>
typename plStaticBitfield<Container>::ConstIterator end(const plStaticBitfield<Container>& container)
{
  return container.GetEndIterator();
}

template <typename Container>
typename plStaticBitfield<Container>::ConstIterator cend(const plStaticBitfield<Container>& container)
{
  return container.GetEndIterator();
}
