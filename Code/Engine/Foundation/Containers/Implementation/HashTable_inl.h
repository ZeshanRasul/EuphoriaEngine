...
plHashTableBaseConstIterator<K, V, H>::plHashTableBaseConstIterator(const plHashTableBase<K, V, H>& hashTable)
  : m_pHashTable(&hashTable)
{
}

template <typename K, typename V, typename H>
void plHashTableBaseConstIterator<K, V, H>::SetToBegin()
{
...
template <typename K, typename V, typename H>
inline void plHashTableBaseConstIterator<K, V, H>::SetToEnd()
{
...
template <typename K, typename V, typename H>
PL_FORCE_INLINE bool plHashTableBaseConstIterator<K, V, H>::IsValid() const
{
  return m_uiCurrentCount < m_pHashTable->m_uiCount;
}

template <typename K, typename V, typename H>
PL_FORCE_INLINE bool plHashTableBaseConstIterator<K, V, H>::operator==(const plHashTableBaseConstIterator<K, V, H>& rhs) const
{
  return m_uiCurrentIndex == rhs.m_uiCurrentIndex && m_pHashTable->m_pEntries == rhs.m_pHashTable->m_pEntries;
}

template <typename K, typename V, typename H>
PL_ALWAYS_INLINE const K& plHashTableBaseConstIterator<K, V, H>::Key() const
{
  return m_pHashTable->m_pEntries[m_uiCurrentIndex].key;
}

template <typename K, typename V, typename H>
PL_ALWAYS_INLINE const V& plHashTableBaseConstIterator<K, V, H>::Value() const
{
  return m_pHashTable->m_pEntries[m_uiCurrentIndex].value;
}

template <typename K, typename V, typename H>
void plHashTableBaseConstIterator<K, V, H>::Next()
{
...
template <typename K, typename V, typename H>
PL_ALWAYS_INLINE void plHashTableBaseConstIterator<K, V, H>::operator++()
{
  Next();
}
...
{
  template <typename K, typename V, typename H>
  struct tuple_size<plHashTableBaseConstIterator<K, V, H>> : integral_constant<size_t, 2>
  {
  };

  template <typename K, typename V, typename H>
  struct tuple_element<0, plHashTableBaseConstIterator<K, V, H>>
  {
    using type = const K&;
  };

  template <typename K, typename V, typename H>
  struct tuple_element<1, plHashTableBaseConstIterator<K, V, H>>
  {
    using type = const V&;
  };
} // namespace std
...
template <typename K, typename V, typename H>
plHashTableBaseIterator<K, V, H>::plHashTableBaseIterator(const plHashTableBase<K, V, H>& hashTable)
  : plHashTableBaseConstIterator<K, V, H>(hashTable)
{
}

template <typename K, typename V, typename H>
plHashTableBaseIterator<K, V, H>::plHashTableBaseIterator(const plHashTableBaseIterator<K, V, H>& rhs)
  : plHashTableBaseConstIterator<K, V, H>(*rhs.m_pHashTable)
{
...
template <typename K, typename V, typename H>
PL_ALWAYS_INLINE void plHashTableBaseIterator<K, V, H>::operator=(const plHashTableBaseIterator& rhs) // [tested]
{
...
template <typename K, typename V, typename H>
PL_FORCE_INLINE V& plHashTableBaseIterator<K, V, H>::Value()
{
  return this->m_pHashTable->m_pEntries[this->m_uiCurrentIndex].value;
}

template <typename K, typename V, typename H>
PL_FORCE_INLINE V& plHashTableBaseIterator<K, V, H>::Value() const
{
  return this->m_pHashTable->m_pEntries[this->m_uiCurrentIndex].value;
}
...
{
  template <typename K, typename V, typename H>
  struct tuple_size<plHashTableBaseIterator<K, V, H>> : integral_constant<size_t, 2>
  {
  };

  template <typename K, typename V, typename H>
  struct tuple_element<0, plHashTableBaseIterator<K, V, H>>
  {
    using type = const K&;
  };

  template <typename K, typename V, typename H>
  struct tuple_element<1, plHashTableBaseIterator<K, V, H>>
  {
    using type = V&;
  };
} // namespace std
