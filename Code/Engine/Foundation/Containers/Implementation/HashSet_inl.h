...
template <typename K, typename H>
template <typename CompatibleKeyType>
PL_FORCE_INLINE typename plHashSetBase<K, H>::ConstIterator plHashSetBase<K, H>::Find(const CompatibleKeyType& key) const
{
  plUInt32 uiIndex = FindEntry(key);
  if (uiIndex == plInvalidIndex)
  {
...
{
  PL_ASSERT_DEBUG(uiEntryIndex < m_uiCapacity, "Out of bounds access");
