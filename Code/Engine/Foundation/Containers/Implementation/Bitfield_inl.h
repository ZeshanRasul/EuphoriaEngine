...
template <class Container>
void plBitfield<Container>::FlipBit(plUInt32 uiBit)
{
  PL_ASSERT_DEBUG(uiBit < m_uiCount, "Cannot access bit {0}, the bitfield only has {1} bits.", uiBit, m_uiCount);
...
template <class Container>
void plBitfield<Container>::FlipBitRange(plUInt32 uiFirstBit, plUInt32 uiNumBits)
{
  if (m_uiCount == 0 || uiNumBits == 0)
    return;
...
template <class Container>
void plBitfield<Container>::Swap(plBitfield<Container>& other)
{
...
template <class Container>
PL_ALWAYS_INLINE typename plBitfield<Container>::ConstIterator plBitfield<Container>::GetIterator() const
{
  return ConstIterator(*this);
};

template <class Container>
PL_ALWAYS_INLINE typename plBitfield<Container>::ConstIterator plBitfield<Container>::GetEndIterator() const
{
  return ConstIterator();
};

//////////////////////////////////////////////////////////////////////////
// plBitfield<Container>::ConstIterator

template <class Container>
plBitfield<Container>::ConstIterator::ConstIterator(const plBitfield<Container>& bitfield)
{
...
template <class Container>
PL_ALWAYS_INLINE bool plBitfield<Container>::ConstIterator::IsValid() const
{
  return m_pBitfield != nullptr;
}

template <class Container>
PL_ALWAYS_INLINE plUInt32 plBitfield<Container>::ConstIterator::Value() const
{
  return *m_Iterator + (m_uiChunk << 5);
}

template <class Container>
PL_ALWAYS_INLINE void plBitfield<Container>::ConstIterator::Next()
{
  ++m_Iterator;
...
template <class Container>
PL_ALWAYS_INLINE bool plBitfield<Container>::ConstIterator::operator==(const ConstIterator& other) const
{
...
template <class Container>
PL_ALWAYS_INLINE plUInt32 plBitfield<Container>::ConstIterator::operator*() const
{
  return Value();
}

template <class Container>
PL_ALWAYS_INLINE void plBitfield<Container>::ConstIterator::operator++()
{
  Next();
}

template <class Container>
void plBitfield<Container>::ConstIterator::FindNextChunk(plUInt32 uiStartChunk)
{
  if (uiStartChunk < m_pBitfield->m_Container.GetCount())
  {
    const plUInt32 uiLastChunk = m_pBitfield->m_Container.GetCount() - 1;
    for (plUInt32 i = uiStartChunk; i < uiLastChunk; ++i)
    {
    ...
    const plUInt32 uiMask = 0xFFFFFFFF >> (32 - (m_pBitfield->m_uiCount - (uiLastChunk << 5)));
...
template <typename T>
PL_ALWAYS_INLINE void plStaticBitfield<T>::Swap(plStaticBitfield<T>& other)
{
  plMath::Swap(m_Storage, other.m_Storage);
}
