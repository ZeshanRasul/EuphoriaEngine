...
public:
  ...
  struct IteratorBase
  {
    ...
    PL_ALWAYS_INLINE bool operator==(const typename plSetBase<KeyType, Comparer>::IteratorBase<REVERSE>& it2) const { return (m_pElement == it2.m_pElement); }
    PL_ADD_DEFAULT_OPERATOR_NOTEQUAL(const typename plSetBase<KeyType, Comparer>::IteratorBase<REVERSE>&);
    ...
    /// \brief Returns the 'key' of the element that this iterator points to.
    PL_ALWAYS_INLINE const KeyType& operator*() const { return Key(); }
  ...
  protected:
    void Advance(plInt32 dir0, plInt32 dir1);

    friend class plSetBase<KeyType, Comparer>;

    PL_ALWAYS_INLINE explicit IteratorBase(Node* pInit)
      : m_pElement(pInit)
    {
    }
