...
# ## WebGPU support
# #####################################
set(PL_BUILD_EXPERIMENTAL_WEBGPU OFF CACHE BOOL "Whether to enable experimental / work-in-progress WebGPU code (please don't)")

# #####################################
# ## pl_requires_webgpu()
# #####################################
macro(pl_requires_webgpu)
    pl_requires(PL_CMAKE_PLATFORM_SUPPORTS_WEBGPU)
    pl_requires(PL_BUILD_EXPERIMENTAL_WEBGPU)
endmacro()

# #####################################
# ## pl_link_target_webgpu(<target>)
# #####################################
function(pl_link_target_webgpu TARGET_NAME)
    pl_requires_webgpu()

    if (COMMAND pl_platformhook_link_target_webgpu)
        # call platform-specific hook for linking with WebGPU
        pl_platformhook_link_target_webgpu(${TARGET_NAME})
    endif()
endfunction()
...
# #####################################
# ## pl_webgpu_install_dawn()
# #####################################
function(pl_webgpu_install_dawn)
    pl_requires_webgpu()
    pl_requires(PL_CMAKE_PLATFORM_WINDOWS_DESKTOP) # we only support WebGPU emulation on Windows
    get_property(PL_CMAKE_DAWN_INSTALLED GLOBAL PROPERTY PL_CMAKE_DAWN_INSTALLED)

    if (PL_CMAKE_DAWN_INSTALLED)
        return()
    endif()

    set_property(GLOBAL PROPERTY PL_CMAKE_DAWN_INSTALLED TRUE)
...
# #####################################
# ## pl_link_target_dawn(TARGET_NAME)
# #####################################
function(pl_link_target_dawn TARGET_NAME)
