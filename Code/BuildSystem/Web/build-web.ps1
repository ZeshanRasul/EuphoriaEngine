...
function GenerateCMakeSolution() {
    ...
    $CMAKE_ARGS += "-DPL_SOLUTION_NAME:STRING='Web'"
    $CMAKE_ARGS += "-DPL_BUILD_EXPERIMENTAL_WEBGPU:BOOL=ON" # need WebGPU
    
    # disable lots of stuff that we don't need yet, for a minimal build
    $CMAKE_ARGS += "-DPL_ENABLE_FOLDER_UNITY_FILES:BOOL=OFF" # slower builds, but better error messages
    $CMAKE_ARGS += "-DPL_ENABLE_QT_SUPPORT:BOOL=OFF" # disable QT
    $CMAKE_ARGS += "-DPL_BUILD_FILTER='Everything'"
    $CMAKE_ARGS += "-DCMAKE_BUILD_TYPE=$BuildType" # Debug build for now
    $CMAKE_ARGS += "-DPL_BUILD_FMOD:BOOL=OFF" # FMOD not yet supported
    # $CMAKE_ARGS += "-DPL_BUILD_RMLUI:BOOL=OFF" # currently not necessary to set this variable
    $CMAKE_ARGS += "-DPL_BUILD_SAMPLES:BOOL=ON"
    $CMAKE_ARGS += "-DPL_BUILD_UNITTESTS:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_ADS_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_ASSIMP_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_DUKTAPE_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_ENET_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_IMGUI_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_JOLT_SUPPORT:BOOL=OFF" # Jolt not yet supported
    $CMAKE_ARGS += "-DPL_3RDPARTY_KRAUT_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_LUA_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_OZZ_SUPPORT:BOOL=ON"
    $CMAKE_ARGS += "-DPL_3RDPARTY_RECAST_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DPL_3RDPARTY_TINYEXR_SUPPORT:BOOL=OFF"
    Write-Host ""
