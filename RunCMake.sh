...
#Debian based distros use fantasy package names instead of the original upstream ones
#This is a problem because distros and their descendents, e.g. Arch or Void, use the originals 
#Often this can be solved by putting -devel where debian distros just put -dev, but some packages
#e.g. cmake, have the same name across distros, where others have different capitalisation,
#e.g. libxinerama-dev vs. libXinerama-devel... Other times times Debian will bundle multiple
#upstream packages into a single fantasy package, in which case you'll need to know all the equivalent
#upstream packages to match the fantasy package! Ahh!!! When in doubt just open an issue about
#it if you're unable to figure out a package name
if [ "$Distribution" = "Ubuntu" ]\
|| [ "$Distribution" = "Pop!_OS" ]\
|| [ "$Distribution" = "Mint" ]; then
  echo "Using made up Debian fantasy package names..."
  packages=(cmake build-essential ninja-build libxrandr-dev libxinerama-dev libxcursor-dev libxi-dev uuid-dev mold libfreetype-dev libtinfo5 libomp-dev)
  ...
  packages=(cmake ninja qt6-base-devel qt6-base-private-devel qt6-svg-devel libXrandr-devel libXinerama-devel libXcursor-devel libXi-devel libuuid-devel mold freetype-devel ncurses-libtinfo-devel libomp-devel)
