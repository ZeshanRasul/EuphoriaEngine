...
Texture2D SnowDiffuseTexture;
Texture2D SnowNormalTexture;
Texture2D SnowORMTexture;
Texture2D SnowNoiseTexture;
...
float EvaluatePBRLight(float3 worldPosition, float3 worldNormal, plPerLightData lightData, uint type, out float3 lightVector, out float attenuation, out float distanceToLight)
{
...
void EvaluateFillLight(float3 worldPosition, float3 worldNormal, float3 diffuseColor, float directionality, plPerLightData lightData, uint type, inout float3 diffuseLight, inout float3 indirectLightModulation)
{
...
// debug cascade or point face selection
  ...
  totalLight.diffuseLight += matData.diffuseColor * (indirectLightModulation * skyLight * kD) * occlusion;

  const float mipLevel = MipLevelFromRoughness(matData.roughness, 8);

  float4 ssr = SSRTexture.SampleLevel(PointClampSampler, float3(float2(screenPosition.xy * ViewportSize.zw), s_ActiveCameraEyeIndex), mipLevel);

  float3 skyboxColor = matData.specularColor * indirectLightModulation * ComputeReflection(matData, viewVector, clusterData) * occlusion;
  float3 ssrColor = matData.specularColor * indirectLightModulation * ssr.xyz * occlusion;
  // indirect specular
  totalLight.specularLight +=  lerp(skyboxColor, ssrColor, ssr.w) ;
  ...
  totalLight.specularLight *= indirectLightModulation * occlusion;
...
float3 SnowTangentToWorldSpace(float3 normalTS, float3 normal)
{

  return normalTS.z * normal;
}

void ApplySnow(inout plMaterialData matData, float3 normal, float4 vertexColor)
{

  float transitionNoise = SampleTexture3WayStochastic(SnowNoiseTexture, SnowSampler, matData.worldNormal,  matData.worldPosition, SnowGlobalScale, true).x;
  float variationNoise = SampleTexture3WayStochastic(SnowNoiseTexture, SnowSampler, matData.worldNormal,  matData.worldPosition, SnowBreakupTiling, true).x;

  float3 upDir = float3(0,0,1);

  float snowBlend = dot(matData.worldNormal, upDir);
  float snowStrength = max(0.0, SnowGlobalStrength - ((SnowGlobalStrength * SnowBreakup) * variationNoise));

  snowBlend = saturate((snowBlend + (SnowGlobalTransition * transitionNoise) ) * snowStrength);

#if defined(USE_WEATHER_VERTEX_COLOR)
  snowBlend *= vertexColor.r;
#endif


  float4 snowColor = SampleTexture3WayStochastic(SnowDiffuseTexture, SnowSampler, normal, matData.worldPosition, SnowTiling, true);
  matData.diffuseColor = lerp(matData.diffuseColor, snowColor, snowBlend);

#if defined(USE_NORMAL)
  float3 xDir = float3(1,0,0);
  float3 snowNormal;

  float3 snowNormalTS = DecodeNormalTexture(SampleTexture3WayStochastic(SnowNormalTexture, SnowSampler, matData.vertexNormal, matData.worldPosition, SnowTiling, true));
  snowNormal = snowNormalTS.x * xDir;
  snowNormal += snowNormalTS.y * cross(matData.vertexNormal, xDir);
  snowNormal += snowNormalTS.z * matData.vertexNormal;


  matData.worldNormal = normalize(lerp(matData.worldNormal, snowNormal, snowBlend));
#endif

  float4 snowORM = SampleTexture3WayStochastic(SnowORMTexture, SnowSampler, matData.worldNormal, matData.worldPosition, SnowTiling, true);
  matData.occlusion = lerp(matData.occlusion, 1.0, snowBlend);
  matData.roughness = lerp(matData.roughness, snowORM.y, snowBlend);
  matData.metalness = lerp(matData.metalness, snowORM.z, snowBlend);

  float3 snowSpecularColor = lerp(0.04f, snowColor, snowORM.z);
  matData.specularColor = lerp(matData.specularColor, snowSpecularColor, snowBlend);
}
...
float4 CalculateRefractionBlur(float3 worldPosition, float3 worldNormal, float IoR, float thickness, float3 tintColor, float radius, float strength, float newOpacity = 1.0f)
{
  float3 normalizedViewVector = normalize(GetCameraPosition() - worldPosition);
  float r = 1.0f / IoR;
  float NdotV = dot(worldNormal, normalizedViewVector);
    ...
    for (int i = -radius; i <= radius; i++)
  {
    ...
    {
      float2 offset = float2(i, j) * vPixelSize;
      color += SceneColor.Sample(SceneColorSampler, float3(refractCoords + offset, s_ActiveCameraEyeIndex));
      weightSum += 1;
    }
  }

  float4 originalColor = SceneColor.Sample(SceneColorSampler, float3(refractCoords, s_ActiveCameraEyeIndex));

  color /= weightSum;
  color = lerp(originalColor, color, saturate(strength));

  float fresnel = pow(1.0f - NdotV, 5.0f);
  color.xyz *= tintColor * (1.0f - fresnel);

  return float4(color.xyz, newOpacity);
}
