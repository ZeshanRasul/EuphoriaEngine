...
PS_OUT main(PS_IN Input)
{
  ...
  uint gameObjectId = GetInstanceData().GameObjectID;
    ...
    ApplySnow(matData, normal, vertexColor);
  #endif
    ...
    //   float3 sceneColor = Blur(Input.WorldPosition, matData.blurSamples, matData.roughness * matData.blurMultiplier);
    //   light.diffuseLight = lerp(sceneColor, light.diffuseLight + light.specularLight, matData.opacity);
    //   matData.opacity = 1.0;
    // #endif
 ...
 //   float packLastXY = pack_floats(Input.LastScreenPosition.xy);

    Output.Color = float4(litColor, matData.opacity);
    Output.Velocity = float4(matData.velocity, 0.0f, 0.0f);
